...
class SMC100CC:
    ...
    def send_command_listen(self, command, value, time):
        ...
        response = str(self.port.read(self.port.inWaiting()), encoding='ascii', errors='ignore')
        if self.DEBUG:
    ...
    def get_pos(self, retry=True):
        ...
        try:
            ...
            if retry:
                ...
                self.port.open()
                sleep(0.1)
                return self.get_pos(retry = False)
            else:
                raise
    ...
    def get_state(self):
        ...
        # state_code = "" #test
        state_descr = self.state_code_table.get(state_code, "")
