...
if __name__ == '__main__':
    ...
    for i, arg in enumerate(sys.argv):
        print(f"arg[{i}] = {arg}")
    if len(sys.argv) != 2:
        print("Not enought arguments. Breaking...")
        raise ValueError
    
    fn_in = sys.argv[1]
    fn_out = 'output_'+fn_in.replace('.npz', '.txt')
    fn_pickle = fn_in.replace('.npz', '.pickle')
    print(fn_in, '->', fn_out)
    print("-----------")
    
    test_data = np.load(fn_in)
    test_tomograms = test_data['test_data']
    PICKLE = fn_pickle
    if not(PICKLE in os.listdir()):
        cost_functions = []
        ...
        with open(PICKLE, 'wb') as jar:
            pickle.dump(cost_functions, jar)
    else:
        with open(PICKLE, 'rb') as jar:
